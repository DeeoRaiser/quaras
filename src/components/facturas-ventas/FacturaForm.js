...
const [loading, setLoading] = useState(false);
  ...
  const calcularTotalLinea = (item) => {
    const base = toNumber(item.precio_venta) * toNumber(item.cantidad);
  ...
  // ───── FETCHS INICIALES ─────
  useEffect(() => {
    fetch("/api/clientes").then(r => r.json()).then(setClientes).catch(() => setClientes([]));
    fetch("/api/articulos").then(r => r.json()).then(setProductos).catch(() => setProductos([]));
    fetch("/api/puntos-venta/factura").then(r => r.json()).then(setPuntosVenta).catch(() => setPuntosVenta([]));
    fetch("/api/impuestos/factura_venta").then(r => r.json()).then(setImpuestosDisponibles).catch(() => setImpuestosDisponibles([]));
  }, []);

  useEffect(() => {
    if (!puntoVenta) return setLetrasDisponibles([]);
    const letras = puntosVenta
      .filter(p => toNumber(p.punto_venta) === toNumber(puntoVenta))
      .map(p => p.letra);
    setLetrasDisponibles(letras);
  }, [puntoVenta, puntosVenta]);

  useEffect(() => {
    if (!puntoVenta || !letra) return;
    fetch(`/api/facturas-ventas/proximo-numero?punto=${puntoVenta}&letra=${letra}`)
      .then(r => r.json())
      .then(data => setNumeroFactura(data.numero || ""))
      .catch(() => setNumeroFactura(""));
  }, [puntoVenta, letra]);
  ...
  const agregarArticulo = (prod) => {
    ...
    setDetalle(prev => [
      ...prev,
      {
        ...
        precio_venta: toNumber(prod.precio_venta),
        cantidad: 1,
        ajuste: 0,
  ...
  const actualizarItem = (i, campo, valor) => {
    const normalized =
      (campo === "precio_venta" || campo === "cantidad" || campo === "ajuste") ? toNumber(valor) : valor;
  ...
  const subtotalSinIVA = detalle.reduce(
    (acc, item) =>
      acc + (
        (toNumber(item.precio_venta) * toNumber(item.cantidad)) / (1 + (toNumber(item.iva || 0) / 100))
      ),
    0
  );
  ...
  const guardarFactura = async (pagoData) => {
    ...
    try {
      const facturaRes = await fetch("/api/facturas-ventas", {
        method: "POST",
      ...
      if (!facturaRes.ok) {
        ...
        throw new Error(text || "Error creando factura");
      }
  ...
  return (
    <Paper sx={{ p: 3, mt: 2 }}>
      <Typography variant="h5" mb={3}>Crear Factura VENTAS</Typography>

      {/* CLIENTE */}
      <Autocomplete
        options={clientes}
        getOptionLabel={(option) => option.nombre || ""}
        value={clientes.find(c => c.id === cliente_id) || null}
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
        ...
        >
          {[...new Set(puntosVenta.map(p => p.punto_venta))].map((pv) => (
      ...
      <Autocomplete
        ...
        getOptionLabel={(option) =>
          option ? `${option.descripcion} - $${option.precio_venta}` : ""
        }
        renderInput={(params) =>
          <TextField {...params} label="Buscar artículo" fullWidth />
        }
        sx={{ mb: 3 }}
      />
      ...
      {detalle.map((item, i) => (
        <Box key={i} sx={{
          ...
          <TextField
            ...
            onChange={(e) => actualizarItem(i, "precio_venta", e.target.value)}
          />
